...
get_node_url(){
	...
	nodes=(http://128.1.164.196 http://45.76.53.20 http://103.224.251.67 http://dg2.bt.cn http://dg1.bt.cn http://123.129.198.197 http://125.88.182.172:5880 http://119.188.210.21:5880 http://120.206.184.160 http://113.107.111.78 http://183.235.223.101:3389);
...
Set_Bt_Panel(){
	...
	if [ -z "${isStart}" ];then
		Red_Error "ERROR: The aaPanel service startup failed."
	fi
}
Set_Firewall(){
...
+----------------------------------------------------------------------
"
curl -sS --connect-timeout 10 -m 60 http://www.aapanel.com/Api/SetupCount?type=Linux > /dev/null 2>&1
while [ "$go" != 'y' ] && [ "$go" != 'n' ]
do
...
echo -e "=================================================================="
if [ "$SET_SSL" == true ];then
        echo  "aaPanel: https://${getIpAddress}:${panelPort}$auth_path"
else
        echo "aaPanel: http://${getIpAddress}:${panelPort}$auth_path"
fi
